...
switch (_code) do {
    ...
    case 34: {
        if (_shift) then {_handled = true;};
        if (_shift && playerSide isEqualTo civilian && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1  && (player distance (getMarkerPos "marker_222") > 650)  && (player distance (getMarkerPos "marker_223") > 450)  && (player distance (getMarkerPos "marker_224") > 150)  && (player distance (getMarkerPos "marker_225") > 300)  && (player distance (getMarkerPos "marker_227") > 400) && (player distance (getMarkerPos "marker_228") > 70) && (player distance (getMarkerPos "marker_229") > 350) && (player distance (getMarkerPos "marker_231") > 300) && (player distance (getMarkerPos "marker_233") > 150) && (player distance (getMarkerPos "marker_238") > 230) && (player distance (getMarkerPos "marker_239") > 150) && (player distance (getMarkerPos "marker_249") > 80) && (player distance (getMarkerPos "marker_250") > 80) && (player distance (getMarkerPos "marker_253") > 150)) then {
	...
	case 207:
    {
		switch (player getVariable["Earplugs",0]) do {
			case 100: {hintSilent "Ear Plugs 0%"; 1 fadeSound 0; player setVariable ["Earplugs", 0]; };
			case 0: {hintSilent "Ear Plugs 10%"; 1 fadeSound 0.1; player setVariable ["Earplugs", 10]; };
			case 10: {hintSilent "Ear Plugs 40%"; 1 fadeSound 0.4; player setVariable ["Earplugs", 40]; };
			case 40: {hintSilent "Ear Plugs 70%"; 1 fadeSound 0.7; player setVariable ["Earplugs", 70]; };
			case 70: {hintSilent "Ear Plugs Removed"; 1 fadeSound 1; player setVariable ["Earplugs", 100]; };
		    };
	};
	...
	case 82:
	{
		if(_shift) then {_handled = true;};
			...
			{
				sleep 10;
				cutText [format["Motion End"], "PLAIN DOWN"];
